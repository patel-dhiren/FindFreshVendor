...
class FirebaseService {
  ...
  Future<void> createUserWithEmailAndPassword(AppUser user) async {
    try {
  ...
  Future<void> updateVendorProfile(AppUser user) async {
  ...
  Future<bool?> addOrUpdateItem(
    ...
    try {
      ...
      if (itemId == null) {
        var id = _database.ref().child('items/$vendorId').push().key;
        item.id = id;
        await _database.ref().child('items/$vendorId').child(id!).set(item.toJson());
      } else {
        // Update existing category
        await _database
            .ref()
            .child('items/$vendorId')
            .child(itemId)
            .update(item.toJson());
      }
  ...
  Future<AppUser> fetchVendorById() async {
    try {
      ...
      DataSnapshot snapshot = await _database.ref().child('vendors/$vendorId').get();
      if (snapshot.exists) {
        Map<dynamic, dynamic> vendorData = Map<dynamic, dynamic>.from(snapshot.value as Map);
